...
def test_index(client, auth):
    ...
    assert b'by <span class="text-info">test</span> on <span class="text-muted">2018-01-01</span>' in response.data
    assert b'test\nbody' in response.data
    assert b'href="/1/update"' in response.data


# A user must be logged in to access the create, update, and delete views.
# The logged in user must be the author of the post to access update and delete, otherwise a 403 status is returned.
